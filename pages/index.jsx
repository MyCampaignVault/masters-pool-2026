...
function matchGolfer(pickName, espnGolfers) {
  ...
  if (lastMatches.length > 1) {
    ...
    const firstMatch = lastMatches.find((g) =>
      normalize(g.name).startsWith(pickFirst)
    );
...
export default function MastersPool() {
  ...
  const poolStandings = POOL_PLAYERS.map((player) => {
    ...
    return {
      ...player,
      totalScore,
      bestRound: bestRound === Infinity ? null : bestRound,
      cutsMade,
      matched,
      golferDetails,
    };
  ...
  const s = {
    page: {
      minHeight: "100vh",
      background: "linear-gradient(160deg, #0a1a0f 0%, #0f2415 40%, #081810 100%)",
      fontFamily: "'Playfair Display', Georgia, serif",
      color: "#f5e6c8",
    },
    header: {
      background: "linear-gradient(180deg, #1a3a1f 0%, #0f2415 100%)",
    ...
    headerPattern: {
      ...
      backgroundImage: "repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(201,168,76,0.03) 10px, rgba(201,168,76,0.03) 20px)",
    },
    ...
    liveBar: {
      ...
      borderBottom: "1px solid rgba(201,168,76,0.15)", fontSize: "12px",
    },
    liveDot: {
      width: "8px", height: "8px", borderRadius: "50%",
      background: "#6bcf7f", boxShadow: "0 0 8px #6bcf7f",
      animation: "pulse 2s infinite",
    },
    tab: (active) => ({
      flex: 1, padding: "12px", background: "transparent",
      border: "none", borderBottom: active ? "3px solid #c9a84c" : "3px solid transparent",
      color: active ? "#c9a84c" : "#a8c5a0",
      ...
      <div style={s.page}>
        ...
        <div style={{ padding: "20px 16px", maxWidth: "940px", margin: "0 auto" }}>
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", padding: "14px 16px", gap: "12px" }}>
                    <div style={{
                      ...
                      color: i === 0 ? "#0a1a0f" : "#f5e6c8",
                      flexShrink: 0,
                    }}>
                      {i + 1}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: "16px", fontWeight: "bold", color: i < 2 ? "#c9a84c" : "#f5e6c8" }}>
                        {player.name}
                      </div>
                      <div style={{ fontSize: "11px", color: "#a8c5a0", marginTop: "2px" }}>
                        {player.cutsMade}/{player.golfers.length} golfers active &middot; Tap to expand
                      </div>
                    </div>
                    <div style={{
                      fontSize: "22px", fontWeight: "bold",
                      color: scoreColor(player.totalScore),
                      ...s.mono,
                    }}>
                  ...
                  {expandedPlayer === player.name && (
                    <div style={{
                        ...
                        <tbody>
                          {player.golferDetails.map((gd, gi) => (
                            <tr key={gi} style={{
                              ...
                              <td style={{ textAlign: "center", padding: "7px 4px", color: "#a8c5a0", ...s.mono }}>
                                {gd.live?.position || "--"}
                              </td>
                              <td style={{ textAlign: "center", padding: "7px 4px", color: scoreColor(gd.live?.scoreToPar), fontWeight: "bold", ...s.mono }}>
                                {gd.found ? formatScore(gd.live.scoreToPar) : "--"}
                              </td>
                              <td style={{ textAlign: "center", padding: "7px 4px", color: scoreColor(gd.live?.today), ...s.mono }}>
                                {gd.live?.today != null ? formatScore(gd.live.today) : "--"}
                              </td>
                              <td style={{ textAlign: "center", padding: "7px 4px", color: "#a8c5a0", ...s.mono }}>
                                {gd.live?.thru || "--"}
                              </td>
                      ...
                      {player.golferDetails.some((g) => !g.found) && (
                        <p style={{ fontSize: "10px", color: "#a8c5a0", marginTop: "8px", marginBottom: 0 }}>
                          * Golfer not found in tournament field — scored as E (even par)
                        </p>
                      )}
                    </div>
                  )}
          ...
          {activeTab === "rosters" && (
            <div>
              <p style={{ color: "#a8c5a0", fontSize: "13px", marginTop: 0, marginBottom: "16px" }}>
                Each player&apos;s 9 golfer picks for The Masters 2026.
              </p>
                    ...
                    <div key={player.name} style={s.card}>
                      <div style={{
                        ...
                        <div style={{
                          ...
                          color: scoreColor(standing?.totalScore),
                          ...s.mono,
                        }}>
                          {standing ? formatScore(standing.totalScore) : "--"}
                        </div>
                      </div>
                        ...
                        {player.golfers.map((g, i) => {
                          ...
                          return (
                            <div key={i} style={{
                              ...
                              padding: "5px 0",
                              borderBottom: i < player.golfers.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none",
                              opacity: live?.isCut ? 0.5 : 1,
                            }}>
                              <span style={{ fontSize: "13px", color: live?.isCut ? "#a8c5a0" : "#f5e6c8" }}>
                                {g}
                                {live?.isCut && <span style={{ fontSize: "10px", color: "#e05c5c", marginLeft: "6px" }}>CUT</span>}
                              </span>
                              <span style={{
                                fontSize: "13px", fontWeight: "bold",
                                color: scoreColor(live?.scoreToPar),
                                ...s.mono,
                              }}>
                                {live ? formatScore(live.scoreToPar) : "--"}
                              </span>
                            </div>
                          );
                        })}
          ...
          {activeTab === "golfers" && (
            <div>
              <p style={{ color: "#a8c5a0", fontSize: "13px", marginTop: 0, marginBottom: "16px" }}>
                Full tournament leaderboard — live from ESPN.
              </p>
                        ...
                        .map((golfer, i) => {
                          ...
                          return (
                            <tr key={i} style={{
                              ...
                              opacity: golfer.isCut ? 0.5 : 1,
                            }}>
                              <td style={{ padding: "8px 6px", color: "#a8c5a0", ...s.mono, fontSize: "12px" }}>
                                {golfer.position || i + 1}
                              </td>
                              <td style={{ padding: "8px 6px", color: "#f5e6c8" }}>
                                {golfer.name}
                                {golfer.isCut && <span style={{ fontSize: "10px", color: "#e05c5c", marginLeft: "6px" }}>CUT</span>}
                              </td>
                              <td style={{ textAlign: "center", padding: "8px 6px", fontWeight: "bold", color: scoreColor(golfer.scoreToPar), ...s.mono }}>
                                {formatScore(golfer.scoreToPar)}
                              </td>
                              <td style={{ textAlign: "center", padding: "8px 6px", color: scoreColor(golfer.today), ...s.mono }}>
                                {golfer.today != null ? formatScore(golfer.today) : "--"}
                              </td>
                              <td style={{ textAlign: "center", padding: "8px 6px", color: "#a8c5a0", ...s.mono }}>
                                {golfer.thru || "--"}
                              </td>
        ...
        <div style={{ textAlign: "center", padding: "24px 20px", color: "rgba(168,197,160,0.4)", fontSize: "11px" }}>
          ...
          <div>Auto-updates every 60s &middot; Scores via ESPN</div>
        </div>
      </div>
    </>
  );
